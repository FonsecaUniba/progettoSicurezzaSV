...
void test_instant() {
	...
	const float EXPECTED_DISPLACEMENT = (float) sqrt(2);
	const float EXPECTED_VELOCITY = (float) sqrt(0.02);
	const float EXPECTED_ACCELERATION = (float) sqrt(0.0002);
	const float EXPECTED_PRESSURE = 20;
	...
	float returned_pressure = test_i.get_pressure();
