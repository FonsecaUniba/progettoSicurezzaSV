	...
	*/
	...
	void train_classifier(User to_train) {
		...
		try {
			...
			std::string path = "Classifiers/NBayes/" + std::to_string(to_train.id) + ".xml";
	...
	float compute_distances(int userID, Signature to_check, std::string path) {
		...
		try {
			...
			result = receivedResponses.at<float>(0, 0);
			result = (std::isnan(result)) ? 0.0 : 1.0;
		}
		catch (cv::Exception &e) {
			std::cout << e.what() << std::endl;
		}
	...
	bool test_signature(int userID, Signature to_check, float threshold) {
		...
		float confidence = normalbayes_connector::compute_distances(userID, to_check, path);
