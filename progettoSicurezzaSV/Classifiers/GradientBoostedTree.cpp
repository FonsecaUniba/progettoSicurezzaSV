	...
	*/
	...
	void train_classifier(User to_train) {
		...
		try {
			...
			std::string path = "Classifiers/GBTrees/" + std::to_string(id) + ".xml";
	...
	bool test_signature(int userID, Signature to_check, float threshold) {
		//Loads path where GradientBoostedTree are stored
		...
		std::vector<float> distances = gradientboostedtree_connector::compute_distances(userID, to_check, path);
